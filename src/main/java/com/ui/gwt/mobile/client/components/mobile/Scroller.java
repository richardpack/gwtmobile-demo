...
public class Scroller implements Momentum.Delegate, TouchHandler.DragDelegate,
    ...
    private boolean useNativeAnimation;
     ...
     */
    public Scroller(Element frame, Element layer, boolean useNativeAnimation) {
    ...
    /**
     * Modified by rpack to use translate3d
     *
    ...
    public void setContentOffset(double x, double y) {
        contentOffset.x = x;
        contentOffset.y = y;
        if (useNativeAnimation && !BrowserUtils.isAndroid) {
            String value = "translate3d(" + (int) x + "px, " + (int) y + "px, 0)";
            setStyle(layer, value);
        } else {
     ...
     */
    public boolean startDeceleration(Point velocity) {
        if (!shouldScrollHorizontally()) {
            velocity.x = 0;
        }
    ...
    private static native void setStyle(Element element, String style) /*-{
        element.style.webkitTransform = style;
        $wnd.console.log(element.style.webkitTransform);
    }-*/;
}
